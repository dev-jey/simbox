...
from .models import Mod, SubType, Type


class ModCreateForm(forms.ModelForm):
    simulator_ = forms.ModelChoiceField(queryset=Type.objects.all().distinct())
    category_ = forms.ModelChoiceField(queryset=SubType.objects.all().distinct())
    ...
    class Meta:
        model = Mod
        fields = ['title', 'simulator_', 'category_', 'description',
                  'header_image', 'cover_image', 'screenshots', 'mod_file']
        exclude = ('user',)
